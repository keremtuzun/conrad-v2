...
# I5 integrated-mission seeds, version 5 (M1-ACTION-E007): the v4 final_test seeds are SPENT (M1-ACTION-E005)
# and were read before the recovered Model1-to-MCBR calibration-interface repair. Development equals the older
# I5 development split; final_test is fresh. Pinned on 2026-09-23 before any run on it.
...
I5_V7_PARTITIONS_PATH = REPO_ROOT / "configs" / "eval" / "partitions_i5_v7.yaml"
I5_V7_PARTITIONS_SHA256 = "bda2a696da495ce23403cb54c31aa31bfacc48a0fd425be332a57b24046be153"
I5_V7_DOMAIN = "i5_mission_v7"
# I5 Spatial V1 cycle: fresh development, validation and final blocks. The final
# block is deliberately larger so the prospective warrant-incidence power rule
# can select N without allocating or inspecting another seed family.
I5_V8_PARTITIONS_PATH = REPO_ROOT / "configs" / "eval" / "partitions_i5_v8.yaml"
I5_V8_PARTITIONS_SHA256 = "f0a7007deda3470a742a13427ab3618a1c237d1712fcc11df48d55aa15752d29"
I5_V8_DOMAIN = "i5_mission_v8"
# I5 Spatial V1 boundary-feasibility correction: every block is fresh after the
# v8 development pool exposed the sensor-pose versus executed-vehicle-pose gap.
...
# Formal I5 Unity partition v2: the v1 final worlds were touched by the interrupted recovered run. Both
# blocks are disjoint from v1 and were pinned on 2026-09-23 before any world in either block was built.
I5_UNITY_V2_PARTITIONS_PATH = REPO_ROOT / "configs" / "eval" / "partitions_i5_unity_v2.yaml"
I5_UNITY_V2_PARTITIONS_SHA256 = "664edb970ea347df21cf86f9bf50aaa60efd8a72e10ffcdb729b88c4923b91a9"
I5_UNITY_V2_DOMAIN = "i5_unity_v2"
# Spatial V1.1 formal Unity I5 cycle. Both blocks are fresh; only the
# prospectively declared prefix of the final block may be opened.
I5_UNITY_V3_PARTITIONS_PATH = REPO_ROOT / "configs" / "eval" / "partitions_i5_unity_v3.yaml"
I5_UNITY_V3_PARTITIONS_SHA256 = "cb8afdb68cec7e95a013a75b46b31491a382440ef0f37a48cc6e469bf026c31f"
I5_UNITY_V3_DOMAIN = "i5_unity_v3"
...
def load_i5_v5(path: Path = I5_V5_PARTITIONS_PATH, *, verify_digest: bool = True) -> dict[str, Any]:
    """The post-repair I5 v5 partition; its final split is disjoint from every older I5 final."""
    digest = canonical_digest(path)
    if verify_digest and digest != I5_V5_PARTITIONS_SHA256:
        raise PartitionIntegrityError(
            f"{path} changed after freezing: digest {digest} != pinned {I5_V5_PARTITIONS_SHA256}"
        )
    raw = yaml.safe_load(path.read_text(encoding="utf-8"))
    validate_i5_v5(
        raw,
...
def validate_i5_v5(
    ...
    """Final seeds are globally fresh; development may only reuse the common older development split."""
    seeds = raw["world_seeds"]
    dev, final = set(_seeds(seeds["development"])), set(_seeds(seeds["final_test"]))
    if dev & final:
        raise PartitionIntegrityError("i5_v5: final_test and development seeds overlap")
    taken = {
        s
        for domain in ("abstract", "mission")
        for p in Partition
    ...
    if final & (taken | older_dev | older_final):
        raise PartitionIntegrityError("i5_v5 final_test seeds collide with seeds used by another partition")
    if dev & (taken | older_final) or not dev <= older_dev:
        raise PartitionIntegrityError(
            "i5_v5 development seeds must be older I5 development seeds and nothing else"
        )


def _i5_v5_split(part: Partition) -> Split:
    loaded = load_i5_v5()
    raw = loaded["raw"]
    if part.value not in raw["world_seeds"]:
        raise KeyError(f"i5_v5 partition has no {part.value!r} split")
    return Split(
        domain=I5_V5_DOMAIN,
        partition=part,
        world_seeds=_seeds(raw["world_seeds"][part.value]),
...
def load_i5_v7(path: Path = I5_V7_PARTITIONS_PATH, *, verify_digest: bool = True) -> dict[str, Any]:
    """Fresh I5 repair partition, including an untouched validation split."""
    digest = canonical_digest(path)
    if verify_digest and digest != I5_V7_PARTITIONS_SHA256:
        raise PartitionIntegrityError(
            f"{path} changed after freezing: digest {digest} != pinned {I5_V7_PARTITIONS_SHA256}"
        )
    raw = yaml.safe_load(path.read_text(encoding="utf-8"))
    ...
    if set(blocks) != {"development", "validation", "final_test"} or any(not b for b in blocks.values()):
        raise PartitionIntegrityError("i5_v7 requires nonempty development, validation, final_test")
...
def _i5_v7_split(part: Partition) -> Split:
    loaded = load_i5_v7()
    raw = loaded["raw"]
    if part.value not in raw["world_seeds"]:
        raise KeyError(f"i5_v7 partition has no {part.value!r} split")
    return Split(
        domain=I5_V7_DOMAIN,
        partition=part,
        world_seeds=_seeds(raw["world_seeds"][part.value]),
...
def load_i5_v8(path: Path = I5_V8_PARTITIONS_PATH, *, verify_digest: bool = True) -> dict[str, Any]:
    """Fresh I5 Spatial V1 cycle with a sealed, power-sized final pool."""
    digest = canonical_digest(path)
    if verify_digest and digest != I5_V8_PARTITIONS_SHA256:
        raise PartitionIntegrityError(
            f"{path} changed after freezing: digest {digest} != pinned {I5_V8_PARTITIONS_SHA256}"
        )
    raw = yaml.safe_load(path.read_text(encoding="utf-8"))
    blocks = {name: set(_seeds(spec)) for name, spec in raw["world_seeds"].items()}
    if set(blocks) != {"development", "validation", "final_test"} or any(not b for b in blocks.values()):
        raise PartitionIntegrityError("i5_v8 requires nonempty development, validation, final_test")
    if sum(map(len, blocks.values())) != len(set().union(*blocks.values())):
        raise PartitionIntegrityError("i5_v8 splits overlap")
    taken = set().union(*(set(_seeds(item)) for item in raw.get("reserved_elsewhere", [])))
    taken |= set().union(*(set(_seeds(spec)) for spec in load_i5_v7()["raw"]["world_seeds"].values()))
    taken |= set().union(*(set(_seeds(spec)) for spec in load_i5_unity_v2()["raw"]["world_seeds"].values()))
    if set().union(*blocks.values()) & taken:
        raise PartitionIntegrityError("i5_v8 collides with historical or reserved seeds")
    if tuple(raw["scenarios"]) != (
        "I5-NOMINAL",
        "I5-CRITICAL-FINDING",
        "I5-UNCERTAIN-BELIEF",
        "I5-ROUTE-BLOCKED",
        ...
        raise PartitionIntegrityError("i5_v8 must exclude the legacy fake-tile nominal scenario")
    return {"raw": raw, "digest": digest}


def _i5_v8_split(part: Partition) -> Split:
    loaded = load_i5_v8()
    raw = loaded["raw"]
    if part.value not in raw["world_seeds"]:
        raise KeyError(f"i5_v8 partition has no {part.value!r} split")
    return Split(
        domain=I5_V8_DOMAIN,
        partition=part,
        world_seeds=_seeds(raw["world_seeds"][part.value]),
...
def load_i5_v9(path: Path = I5_V9_PARTITIONS_PATH, *, verify_digest: bool = True) -> dict[str, Any]:
    """Fresh I5 Spatial V1 cycle after the boundary-feasibility correction."""
    digest = canonical_digest(path)
    if verify_digest and digest != I5_V9_PARTITIONS_SHA256:
        raise PartitionIntegrityError(
            f"{path} changed after freezing: digest {digest} != pinned {I5_V9_PARTITIONS_SHA256}"
        )
    raw = yaml.safe_load(path.read_text(encoding="utf-8"))
    blocks = {name: set(_seeds(spec)) for name, spec in raw["world_seeds"].items()}
    if set(blocks) != {"development", "validation", "final_test"} or any(not b for b in blocks.values()):
        raise PartitionIntegrityError("i5_v9 requires nonempty development, validation, final_test")
    if sum(map(len, blocks.values())) != len(set().union(*blocks.values())):
        raise PartitionIntegrityError("i5_v9 splits overlap")
    taken = set().union(*(set(_seeds(item)) for item in raw.get("reserved_elsewhere", [])))
    taken |= set().union(*(set(_seeds(spec)) for spec in load_i5_v8()["raw"]["world_seeds"].values()))
    taken |= set().union(*(set(_seeds(spec)) for spec in load_i5_unity_v2()["raw"]["world_seeds"].values()))
    if set().union(*blocks.values()) & taken:
        raise PartitionIntegrityError("i5_v9 collides with historical or reserved seeds")
    if tuple(raw["scenarios"]) != tuple(load_i5_v8()["raw"]["scenarios"]):
        raise PartitionIntegrityError("i5_v9 must retain the legitimate Spatial V1 scenario set")
    return {"raw": raw, "digest": digest}


def _i5_v9_split(part: Partition) -> Split:
    loaded = load_i5_v9()
    raw = loaded["raw"]
    if part.value not in raw["world_seeds"]:
        raise KeyError(f"i5_v9 partition has no {part.value!r} split")
    return Split(
        domain=I5_V9_DOMAIN,
        partition=part,
        world_seeds=_seeds(raw["world_seeds"][part.value]),
...
def load_i5_unity_v2(
    path: Path = I5_UNITY_V2_PARTITIONS_PATH, *, verify_digest: bool = True
) -> dict[str, Any]:
    """Fresh formal Unity gate I5 worlds after the interrupted v1 run spent its declared pair."""
    digest = canonical_digest(path)
    if verify_digest and digest != I5_UNITY_V2_PARTITIONS_SHA256:
        raise PartitionIntegrityError(
            f"{path} changed after freezing: digest {digest} != pinned {I5_UNITY_V2_PARTITIONS_SHA256}"
        )
    raw = yaml.safe_load(path.read_text(encoding="utf-8"))
    validate_i5_unity(
        raw,
...
def _i5_unity_v2_split(part: Partition) -> Split:
    loaded = load_i5_unity_v2()
    raw = loaded["raw"]
    if part.value not in raw["world_seeds"]:
        raise KeyError(f"i5_unity_v2 partition has no {part.value!r} split")
    return Split(
        domain=I5_UNITY_V2_DOMAIN,
        partition=part,
        world_seeds=_seeds(raw["world_seeds"][part.value]),
        families=tuple(raw["families"]),
        replicates_per_world=1,
...
def load_i5_unity_v3(
    path: Path = I5_UNITY_V3_PARTITIONS_PATH, *, verify_digest: bool = True
) -> dict[str, Any]:
    """Fresh Spatial V1.1 formal Unity I5 worlds, disjoint from all older I5 partitions."""
    digest = canonical_digest(path)
    if verify_digest and digest != I5_UNITY_V3_PARTITIONS_SHA256:
        raise PartitionIntegrityError(
            f"{path} changed after freezing: digest {digest} != pinned {I5_UNITY_V3_PARTITIONS_SHA256}"
        )
    raw = yaml.safe_load(path.read_text(encoding="utf-8"))
    validate_i5_unity(
        raw,
...
def _i5_unity_v3_split(part: Partition) -> Split:
    loaded = load_i5_unity_v3()
    raw = loaded["raw"]
    if part.value not in raw["world_seeds"]:
        raise KeyError(f"i5_unity_v3 partition has no {part.value!r} split")
    return Split(
        domain=I5_UNITY_V3_DOMAIN,
        partition=part,
        world_seeds=_seeds(raw["world_seeds"][part.value]),
        families=tuple(raw["families"]),
        replicates_per_world=1,
...
def split(domain: str, partition: str | Partition, purpose: str | Purpose) -> Split:
    ...
    if domain == I5_V7_DOMAIN:
        return _i5_v7_split(part)
    if domain == I5_V8_DOMAIN:
    ...
    if domain == I5_UNITY_V2_DOMAIN:
        return _i5_unity_v2_split(part)
    if domain == I5_UNITY_V3_DOMAIN:
        return _i5_unity_v3_split(part)
...
def partition_of(domain: str, seed: int) -> Partition | None:
    ...
    if domain == I5_V7_DOMAIN:
        v7 = load_i5_v7()["raw"]["world_seeds"]
        return next((Partition(p) for p, spec in v7.items() if int(seed) in _seeds(spec)), None)
    if domain == I5_V8_DOMAIN:
        v8 = load_i5_v8()["raw"]["world_seeds"]
        return next((Partition(p) for p, spec in v8.items() if int(seed) in _seeds(spec)), None)
    if domain == I5_V9_DOMAIN:
    ...
    if domain == I5_UNITY_V2_DOMAIN:
        unity_i5_v2 = load_i5_unity_v2()["raw"]["world_seeds"]
        return next((Partition(p) for p, spec in unity_i5_v2.items() if int(seed) in _seeds(spec)), None)
    if domain == I5_UNITY_V3_DOMAIN:
        unity_i5_v3 = load_i5_unity_v3()["raw"]["world_seeds"]
        return next((Partition(p) for p, spec in unity_i5_v3.items() if int(seed) in _seeds(spec)), None)
    if domain == I7_V2_DOMAIN:
