...
class FakeRobot(RobotHardwareInterface):
    ...
    def get_imu(self) -> ImuSample | None:
        return ImuSample(
            ...
            frame_id="ROBOT",
            linear_acceleration_mps2=(0, 0, 0),
            angular_velocity_rps=(0, 0, 0),
        )

    def get_depth(self) -> DepthSample | None:
        return DepthSample(
            timestamp=self._clock.stamp(), frame_id="ROBOT", depth_m=float(-self.true_position[2])
        )
    ...
    def get_thruster_state(self) -> tuple[ThrusterState, ...]:
        return tuple(
            ThrusterState(thruster_id=k, command=v, estimated_thrust_n=40.0 * v)
...
class FakeFullSystem:
    def __init__(self, repo: Repository, seed: int, log_path: Path | None = None) -> None:
        ...
        self.ctx = RunContext(
            self.run_id, self.mission_id, self.scenario_id, self.ids, self.clock.ns, "SIM", VERSION
        )
    ...
    def estimated_pose(self) -> Pose:
        est = self.robot.true_position + self.rng.normal(
            0, 0.02, 3
        )  # estimator stand-in: noisy, never exact truth
        cov = [0.0] * 36
        cov[0] = cov[7] = cov[14] = 0.02**2
        return Pose(
            frame_id=WORLD,
            position_m=(float(est[0]), float(est[1]), float(est[2])),
            covariance_6x6=tuple(cov),
        )
    ...
    def fake_sensor(self, trace: UUID) -> Observation:
        ...
        reading = (
            self.twin.hidden_severity + float(self.rng.normal(0, 0.03))
            if sees_hidden_side
            else float(self.rng.normal(0.2, 0.2))
        )
        self._seq += 1
        obs = Observation(
            ...
            timestamp=self.clock.stamp(self._seq),
            sensor_frame="SENSOR_CAMERA",
            robot_pose_estimate=self.estimated_pose(),
            inline_values=(float(np.clip(reading, 0, 1)), visible),
            inline_units="unitless,coverage",
        )
    ...
    def fake_ecmer(self, obs: Observation) -> tuple[Evidence, ProvenanceRecord]:
        ...
        ev = Evidence(
            ...
            timestamp=obs.timestamp,
            created_time_ns=max(self.clock.ns(), obs.timestamp.time_ns),
            embedding=(value, coverage, math.sin(value), math.cos(value)),
            reliability=0.3 + 0.65 * coverage,
            aleatoric_uncertainty=0.03 if coverage else 0.2,
            spatial_support=SpatialSupport(
                frame_id=WORLD,
                center_m=(5.0, 0.0, -10.0),
                half_extent_m=(1, 1, 1),
                position_sigma_m=obs.robot_pose_estimate.position_sigma_m(),
            ),
            measurements={"severity": value, "coverage": coverage},
            measurement_units={"severity": "unitless", "coverage": "unitless"},
            independence_group=f"obs:{obs.observation_id}",
            provenance_id=pid,
        ...
        self.log.emit(
            ...
            "fake.ecmer",
            obs.trace_id,
            {"evidence_id": str(eid), "observation_id": str(obs.observation_id)},
            measurement_time_ns=obs.timestamp.time_ns,
            causation=(obs.observation_id,),
        )
        return ev, prov
    ...
    def fake_model2(self, ev: Evidence, eprov: ProvenanceRecord) -> BeliefMessage:
        ...
        if coverage > 0.5:
            ...
            value: float | None = (
                ev.measurements["severity"]
                if old_val is None
                else old_val + weight * (ev.measurements["severity"] - old_val)
            )
        ...
        unc = Uncertainty(
            aleatoric=ev.aleatoric_uncertainty,
            epistemic=1.0 / (1.0 + n),
            contradiction=0.0
            if old_val is None or coverage < 0.5
            else min(1.0, abs(ev.measurements["severity"] - old_val)),
        ...
        claim = PropertyClaim(
            name="severity",
            value=value if status is not KnowledgeStatus.UNKNOWN else None,
            units="unitless",
            status=status,
            uncertainty=unc,
            provenance_id=None if status is KnowledgeStatus.UNKNOWN else pid,
        )
        cell = BeliefCell(
            belief_id=bid,
            domain=Domain.TECHNICAL,
            entity_type="pipeline_segment",
            lifecycle=Lifecycle.CANDIDATE if prior is None else Lifecycle.CONFIRMED,
            revision=rev_no,
            timestamp=ev.timestamp,
            state_embedding=(unc.observational, unc.epistemic, float(n), 0.0),
            claims=(claim,),
            knowledge_status=status,
            uncertainty=unc,
            spatial_support=ev.spatial_support,
            provenance_root=pid,
            model_version=VERSION,
        )
        revision = BeliefRevision(
            belief_id=bid,
            revision=rev_no,
            predecessor_revision=None if prior is None else prior.revision,
            measurement_time_ns=ev.timestamp.time_ns,
            consumed_evidence_ids=(ev.evidence_id,),
            provenance_root=pid,
            update_kind=UpdateKind.DIRECT,
            cell=cell,
        )
        res = self.repo.commit_update(
            BeliefUpdate(
                message_id=self.ids.new(),
                producer_version=VERSION,
                run_id=self.run_id,
                revision=revision,
                provenance=[eprov, bprov],
            )
        )
        self._belief = cell.model_copy(
            update={"independent_observation_count": res.independent_observation_count}
        )
        ...
        msg = BeliefMessage(
            ...
            knowledge_status=status,
            uncertainty=unc,
            evidence_support=(ev.evidence_id,),
            provenance_refs=(pid,),
            spatial_support=cell.spatial_support,
            lifecycle=cell.lifecycle,
            model_version=VERSION,
            technical=TechnicalPayload(
                severity=claim.value if isinstance(claim.value, float) else None, direct_support=best_cov
            ),
        )
        self.result.messages.append(msg)  # FakeBeliefBus
        self.log.emit(
            EventType.BELIEF_PUBLISHED, "fake.bus", ev.trace_id, {"belief_id": str(bid), "revision": rev_no}
        )
        return msg
    ...
    def fake_model1(self, msg: BeliefMessage, trace: UUID) -> tuple[DecisionRecord, InformationNeed | None]:
        claim = DecisionClaim(
            ...
            statement="segment severity",
            structured_value={"status": msg.knowledge_status.value},
            source_belief_ids=(msg.belief_id,),
            source_belief_revisions=(msg.revision,),
            evidence_refs=msg.evidence_support,
        ...
        if msg.uncertainty.observational > 0.5:
            ...
            need = InformationNeed(
                need_id=self.ids.new(),
                trace_id=trace,
                target_belief_ids=(msg.belief_id,),
                question_type=QuestionType.EXTEND_COVERAGE,
                target_properties=("severity",),
                priority=0.9,
                originating_claim_ids=(claim.claim_id,),
            )
        else:
            action = ActionProposal(
                action_id=self.ids.new(),
                action_type=ActionType.TRANSMIT_INFORMATION,
                target_belief_ids=(msg.belief_id,),
                supporting_claims=(claim.claim_id,),
                score=1.0,
            )
        pid = self.ids.new()
        self.repo.put_provenance(
            self.run_id,
            [
                ProvenanceRecord(
                    record_id=pid,
                    source_type=SourceType.DECISION,
                    source_ids=(msg.belief_id,),
                    operation="fake_decide",
                    module="fake.model1",
                    model_version=VERSION,
                    timestamp=msg.timestamp,
                    parent_records=msg.provenance_refs,
        ...
        record = DecisionRecord(
            ...
            mission_id=self.mission_id,
            timestamp=msg.timestamp,
            belief_snapshot_id=msg.message_id,
            claims=(claim,),
            candidates=(action,),
            chosen=action,
            constraint_decisions=(
                ConstraintDecision(action_id=action.action_id, accepted=True, engine_version=VERSION),
            ),
            rationale="coverage missing" if need else "belief resolved",
            provenance_id=pid,
            model_version=VERSION,
        )
        ...
        self.log.emit(
            ...
            trace,
            {
                ...
                "belief_id": str(msg.belief_id),
                "belief_revision": msg.revision,
    ...
    def fake_mcbr(self, need: InformationNeed, decision: DecisionRecord) -> ObservationPlan:
        ...
        self.repo.put_provenance(
            ...
            [
                ProvenanceRecord(
                    ...
                    source_ids=(need.need_id,),
                    operation="fake_plan",
                    module="fake.mcbr",
                    model_version=VERSION,
                    timestamp=self.clock.stamp(),
                    parent_records=(decision.provenance_id,),
                    subject_id=need.need_id,
                )
            ],
        )
        act = ObservationAction(
            ...
            pose=view,
            sensor_id=self.sensor_id,
            target_region=SpatialSupport(frame_id=WORLD, center_m=(5.0, 0.0, -10.0), half_extent_m=(1, 1, 1)),
            duration_s=1.0,
            expected_cost=ResourceCost(time_s=30, energy_j=500, risk=0.05, travel_m=8),
            predicted_visibility=0.9,
            expected_information_gain=0.8,
        )
        plan = ObservationPlan(
            plan_id=self.ids.new(),
            ...
            primary_action=act,
            expected_information_gain=0.8,
            targeted_uncertainty=(UncertaintyType.OBSERVATIONAL,),
            expected_cost=act.expected_cost,
            confidence=0.7,
            provenance=pid,
        )
        ...
        self.log.emit(
            ...
            "fake.mcbr",
            need.trace_id,
            {"plan_id": str(plan.plan_id), "need_id": str(need.need_id), "provenance_id": str(pid)},
        )
        return plan
    ...
    def fake_navigation(self, plan: ObservationPlan) -> None:
        ...
        goal = NavigationGoal(
            ...
            target_pose=plan.primary_action.pose,
        ...
        self.log.emit(
            ...
            "fake.navigation",
            plan.trace_id,
            {"goal_id": str(goal.goal_id), "plan_id": str(plan.plan_id)},
        )
    ...
    def fake_control(self, goal: NavigationGoal, vel: np.ndarray) -> None:
        ...
        wrench = WrenchCommand(
            command_id=self.ids.new(),
            trace_id=goal.trace_id,
            timestamp=self.clock.stamp(),
            frame_id="ROBOT",
            force_n=(float(vel[0]) * 40, float(vel[1]) * 40, float(vel[2]) * 40),
            torque_nm=(0, 0, 0),
        )
        ...
        self.repo.put_provenance(
            ...
            [
                ProvenanceRecord(
                    ...
                    source_ids=(wrench.command_id, goal.goal_id),
                    operation="fake_allocate",
                    module=ALLOCATOR_PRODUCER,
                    model_version=VERSION,
                    timestamp=self.clock.stamp(),
                    parent_records=(self.result.plans[-1].provenance,),
                    subject_id=cid,
                )
            ],
        )
        ...
        self.log.emit(
            ...
            "fake.control",
            goal.trace_id,
            {"wrench_id": str(wrench.command_id), "goal_id": str(goal.goal_id)},
        )
    ...
    def fake_baac(self, msg: BeliefMessage, decision: DecisionRecord) -> None:
        delta = SemanticDelta(
            delta_type=DeltaType.NEW_BELIEF if self._receiver_revision is None else DeltaType.STATE_CHANGED,
            belief_id=msg.belief_id,
            base_revision=self._receiver_revision,
            new_revision=msg.revision,
            changed_fields={
                "severity": msg.state_summary[0].value,
                "status": msg.knowledge_status.value,
                "uncertainty": list(msg.uncertainty.as_tuple()),
            },
        )
        unit = InformationUnit(
            unit_id=self.ids.new(),
            trace_id=decision.trace_id,
            content_type=InformationType.BELIEF_DELTA,
            belief_ids=(msg.belief_id,),
            evidence_ids=msg.evidence_support,
            semantic_delta=delta,
            priority=0.9,
            mission_value=1.0,
            created_time_ns=self.clock.ns(),
            fidelity_levels=(
                FidelityOption(
                    fidelity=Fidelity.F1_STRUCTURED_BELIEF,
                    size_bits=len(delta.canonical_json()) * 8,
        ...
        tx = Transmission(
            transmission_id=self.ids.new(),
            unit_id=unit.unit_id,
            trace_id=unit.trace_id,
            link_name="acoustic",
            fidelity=Fidelity.F1_STRUCTURED_BELIEF,
            bits=unit.fidelity_levels[0].size_bits,
            sent_time_ns=self.clock.ns(),
            delivered=True,
            delivered_time_ns=self.clock.ns() + 1_000_000_000,
    ...
    def run(self, max_cycles: int = 4) -> FakeRunResult:
        ...
        for _ in range(max_cycles):
            ...
            self.clock.t += DT
        self.log.emit(
            EventType.RUN_TERMINATED, "fake.system", self.run_id, {"cycles": len(self.result.decisions)}
        )
