...
def test_i0_loop_closes_and_belief_resolves(tmp_path: Path) -> None:
    system, repo, res = _run(tmp_path, "a")
    ...
    assert {EventType.OBSERVATION_RECEIVED, EventType.EVIDENCE_CREATED, EventType.BELIEF_COMMITTED, EventType.DECISION_MADE,
            EventType.PLAN_PROPOSED, EventType.GOAL_ACCEPTED, EventType.COMMAND_SENT, EventType.COMMAND_ACK, EventType.TRANSMISSION} <= types
...
def test_i0_boundaries_preserve_identity(tmp_path: Path) -> None:
    ...
    for ev in res.events.events:
        ...
        assert ev.envelope.clock_domain == "SIM" and ev.envelope.created_time_ns >= ev.envelope.measurement_time_ns
    for rev in repo.all_revisions(system.run_id):
        ...
        for eid in rev.consumed_evidence_ids:
            ...
            assert obs is not None and obs.timestamp == evidence.timestamp  # measurement time survives unchanged
...
def test_i0_full_causal_trace_from_command_to_raw_observation(tmp_path: Path) -> None:
    system, repo, res = _run(tmp_path, "c")
    ...
    assert {EventType.OBSERVATION_RECEIVED, EventType.DECISION_MADE, EventType.PLAN_PROPOSED, EventType.COMMAND_SENT} <= {e.event_type for e in same_trace}


def test_i0_deterministic_replay(tmp_path: Path) -> None:
    sys_a, repo_a, res_a = _run(tmp_path, "r1", seed=5)
    sys_b, repo_b, res_b = _run(tmp_path, "r2", seed=5)
    assert event_signature(res_a.events.events) == event_signature(res_b.events.events)
    assert [r.canonical_json() for r in repo_a.all_revisions()] == [r.canonical_json() for r in repo_b.all_revisions()]
