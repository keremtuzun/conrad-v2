...
def _create(repo: Repository, ids: IdFactory, run_id, t: float = 1.0, group: str | None = None):
    ...
    upd = BeliefUpdate(message_id=ids.new(), producer_version="t0", run_id=run_id, revision=rev, provenance=[eprov, rprov])
    return bid, ev, upd
...
def test_cc01_duplicate_delivery_is_one_contribution(repo: Repository, ids: IdFactory) -> None:
    ...
    with pytest.raises(DuplicateEvidenceError):
        repo.commit_update(BeliefUpdate(message_id=ids.new(), producer_version="t0", run_id=run_id, revision=rev2, provenance=[p2]))
...
def test_cc01_same_independence_group_is_not_corroboration(repo: Repository, ids: IdFactory) -> None:
    ...
    rev, p = make_revision(ids, bid, 1, 1.2, (ev2.evidence_id, ev3.evidence_id), (eprov2.record_id, eprov3.record_id))
    res = repo.commit_update(BeliefUpdate(message_id=ids.new(), producer_version="t0", run_id=run_id, revision=rev, provenance=[eprov2, eprov3, p]))
...
def test_cc02_late_observation_never_masquerades_as_current(repo: Repository, ids: IdFactory) -> None:
    ...
    with pytest.raises(StaleUpdateError):
        repo.commit_update(BeliefUpdate(message_id=ids.new(), producer_version="t0", run_id=run_id, revision=rev, provenance=[old_prov, p]))
    late_rev, lp = make_revision(ids, bid, 1, 4.0, (old_ev.evidence_id,), (old_prov.record_id,), late=True)
    repo.commit_update(BeliefUpdate(message_id=ids.new(), producer_version="t0", run_id=run_id, revision=late_rev, provenance=[old_prov, lp]))
    ...
    with pytest.raises(PredecessorMismatchError):
        repo.commit_update(BeliefUpdate(message_id=ids.new(), producer_version="t0", run_id=run_id, revision=stale, provenance=[sp]))


@pytest.mark.parametrize("point", ["before_writes", "after_provenance", "after_contributions", "after_revision", "before_commit"])
def test_cc03_ss03_failed_write_leaves_nothing_partial(repo: Repository, ids: IdFactory, point: str) -> None:
    ...
    with pytest.raises(RuntimeError, match="injected failure"):
        repo.commit_update(upd)
    assert repo.counts() == before, "a failed belief write must not leave any revision/provenance/contribution row"
...
def test_cc03_dangling_provenance_rejected(repo: Repository, ids: IdFactory) -> None:
    run_id = ids.new()
    _, _, upd = _create(repo, ids, run_id)
    broken = BeliefUpdate(message_id=upd.message_id, producer_version="t0", run_id=run_id, revision=upd.revision, provenance=[upd.provenance[1]])
    before = repo.counts()
...
def test_cc04_merge_preserves_lineage_to_raw_observations(repo: Repository, ids: IdFactory) -> None:
    ...
    merged = ids.new()
    rev, p = make_revision(
        ids, merged, 0, 3.0, (), (upd_a.revision.provenance_root, upd_b.revision.provenance_root),
        kind=UpdateKind.LIFECYCLE, lifecycle=Lifecycle.CONFIRMED,
    )
    repo.commit_update(BeliefUpdate(message_id=ids.new(), producer_version="t0", run_id=run_id, revision=rev, provenance=[p], lineage_parents=[a, b], lineage_kind="MERGE"))
    for old, upd in ((a, upd_a), (b, upd_b)):
        tomb, tp = make_revision(ids, old, 1, 3.0, (), (upd.revision.provenance_root,), kind=UpdateKind.LIFECYCLE, lifecycle=Lifecycle.MERGED)
        repo.commit_update(BeliefUpdate(message_id=ids.new(), producer_version="t0", run_id=run_id, revision=tomb, provenance=[tp]))
    ...
    with pytest.raises(LifecycleError):
        repo.commit_update(BeliefUpdate(message_id=ids.new(), producer_version="t0", run_id=run_id, revision=zombie, provenance=[zp]))
